...
#define GI2C1_TIMEOUT_US  ((uint32_t)250)  /* number of microseconds as specified in properties */
...
byte GI2C1_ReadBlock(void* data, word dataSize, GI2C1_EnumSendFlags flags)
{
  ...
  for(;;) { /* breaks */
    ...
    res = I2C0_MasterReceiveBlock(GI2C1_deviceData.handle, data, dataSize, flags==GI2C1_SEND_STOP?LDD_I2C_SEND_STOP:LDD_I2C_NO_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
...
byte GI2C1_ReadAddress(byte i2cAddr, byte *memAddr, byte memAddrSize, byte *data, word dataSize)
{
  ...
  for(;;) { /* breaks */
    ...
    GI2C1_deviceData.dataTransmittedFlg = FALSE;
    res = I2C0_MasterSendBlock(GI2C1_deviceData.handle, memAddr, memAddrSize, LDD_I2C_NO_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
    ...
    /* receive data */
    GI2C1_deviceData.dataReceivedFlg = FALSE;
    res = I2C0_MasterReceiveBlock(GI2C1_deviceData.handle, data, dataSize, LDD_I2C_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
    timeout = TMOUT1_GetCounter(GI2C1_TIMEOUT_TICKS(dataSize)); /* set up timeout counter */
...
byte GI2C1_WriteAddress(byte i2cAddr, byte *memAddr, byte memAddrSize, byte *data, word dataSize)
{
  ...
  for(;;) { /* breaks */
    ...
    GI2C1_deviceData.dataTransmittedFlg = FALSE;
    if (I2C0_MasterSendBlock(GI2C1_deviceData.handle, writeBuf, i, LDD_I2C_SEND_STOP)!=ERR_OK) {
      break; /* break for(;;) */
    }
...
{
  GI2C1_deviceData.handle = I2C0_Init(&GI2C1_deviceData);
  if (GI2C1_deviceData.handle==NULL) {
    for(;;){} /* failure! */
  }
  GI2C1_busSem = FRTOS1_xSemaphoreCreateRecursiveMutex();
...
byte GI2C1_ScanDevice(byte i2cAddr)
{
  ...
  for(;;) { /* breaks */
    /* send device address */
    GI2C1_deviceData.dataTransmittedFlg = FALSE;
    res = I2C0_MasterReceiveBlock(GI2C1_deviceData.handle, &dummy, 1, LDD_I2C_SEND_STOP);
    if (res!=ERR_OK) {
      break; /* break for(;;) */
    }
    ...
    errMask = 0;
    (void)I2C0_GetError(GI2C1_deviceData.handle, &errMask);
    TMOUT1_LeaveCounter(timeout);
