...
void CLS1_SendChar(uint8_t ch)
{
  ...
  do {
    res = BT_SendChar((uint8_t)ch);    /* Send char */
    if (res==ERR_TXFULL) {
      WAIT1_WaitOSms(5);
    }
    if(timeoutMs<=0) {
...
bool CLS1_KeyPressed(void)
{
  ...
  res = (bool)((BT_GetCharsInRxBuf()==0U) ? FALSE : TRUE); /* true if there are characters in receive buffer */
  return res;
}
